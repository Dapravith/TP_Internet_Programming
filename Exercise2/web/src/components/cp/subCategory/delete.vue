<template>
    ...
    @ok="hideModal"
    >   
    <h1>Sub Category gonna delete: {{ SubCategoryGonnaDelete.name  }} </h1>
    <h2>Are you sure?</h2>
    <template #footer>
        <a-button key="submit" type="primary" :disabled="isLoadingDelete"  @click="cancelDelete">Cancle</a-button>
        <a-button key="submit" type="danger" :disabled="isLoadingDelete" :loading="isLoadingDelete"   @click="handleDelete(SubCategoryGonnaDelete._id)">Delete</a-button>
    </template>
</a-modal>
</template>
...
const SubCategoryGonnaDelete=ref("")
...
const popUpDelete=(Subcategory)=>{
    SubCategoryGonnaDelete.value=Subcategory
    isDelete.value=true
