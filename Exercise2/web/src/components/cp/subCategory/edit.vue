<template>
    ...
    <!--Confirm-->
    <a-modal v-model:visible="isEdit" :title="`Edit Sub Category | ${subCategoryGonnaEdit.name}` " @ok="handleOk">
        <div class="input" v-if="!isLoadingEdit">
            <label for="name">Sub Category name : </label>
            <div class="name_input">
                
                <input type="text" :class="isError ?'err':''" placeholder="Name" v-model="subCategoryName" required>
        ...
        <template #footer>
            ...
            <a-button key="submit" type="primary" :disabled="isLoadingEdit" :loading="isLoadingEdit" @click="handleEdit(subCategoryGonnaEdit._id)">Save</a-button>
        </template>
    </a-modal>
</template>
...
const subCategoryGonnaEdit=ref("")
...
const popUpDelete=(subCategory)=>{
    subCategoryGonnaEdit.value=subCategory
    subCategoryName.value=subCategoryGonnaEdit.value.name
    isEdit.value=true
...
const handleEdit=(id)=>{
    ...
    fetch(`${store.state.apiUrl}/sub-category/${id}`,{
        method: 'put',
        headers:{
        ...
        body:JSON.stringify({name:subCategoryName.value})
    }).then(res=>res.json())
    .then(data=>{
