<template>
    ...
    <!--Confirm-->
    <a-modal v-model:visible="isEdit" :title="`Edit Product | ${productGonnaEdit.name}` " @ok="handleOk">
        <div class="input" v-if="!isLoadingEdit">
            <label for="name">Product name : </label>
            <div class="name_input">
                
                <input type="text" :class="isError ?'err':''" placeholder="Name" v-model="productName" required>
        ...
        <template #footer>
            ...
            <a-button key="submit" type="primary" :disabled="isLoadingEdit" :loading="isLoadingEdit" @click="handleEdit(productGonnaEdit._id)">Save</a-button>
        </template>
    </a-modal>
</template>
...
const popUpDelete=(product)=>{
    productGonnaEdit.value=product
    productName.value=productGonnaEdit.value.name
    isEdit.value=true
...
const handleEdit=(id)=>{
    ...
    fetch(`${store.state.apiUrl}/product/${id}`,{
        method: 'put',
        headers:{
