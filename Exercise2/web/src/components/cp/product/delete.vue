<template>
    ...
    @ok="hideModal"
    >   
    <h1>Product gonna delete: {{ productGonnaDelete.name  }} </h1>
    <h2>Are you sure?</h2>
    <template #footer>
        <a-button key="submit" type="primary" :disabled="isLoadingDelete"  @click="cancelDelete">Cancle</a-button>
        <a-button key="submit" type="danger" :disabled="isLoadingDelete" :loading="isLoadingDelete"   @click="handleDelete(productGonnaDelete._id)">Delete</a-button>
    </template>
</a-modal>
</template>
...
const popUpDelete=(product)=>{
    productGonnaDelete.value=product
    isDelete.value=true
